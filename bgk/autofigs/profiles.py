...
def plot_profiles(
    ...
    ax.set_xlabel("$\\rho$")
    ax.set_ylabel(videoMaker.param.title)
    ax.set_title(f"Changing Profile of {videoMaker.param.title} for $B_0={videoMaker.params_record.B0}$ {duration_in_title}")
    ax.legend()
    fig.tight_layout()
...
def plot_extrema(
    ...
    ax.set_xlabel("$\\rho$")
    ax.set_ylabel(videoMaker.param.title)
    ax.set_title(f"Extremal Profiles of {videoMaker.param.title} for $B_0={videoMaker.params_record.B0}$")
    ax.legend()
    fig.tight_layout()
