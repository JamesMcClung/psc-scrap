...
class Sequence:
    def __init__(self, n_rows: int, step_idxs: list[int], times: list[float]) -> None:
        ...
        self.fig, self.axs = plt.subplots(n_rows, len(self.step_idxs) + 1, squeeze=False)  # +1 col for the cmap
    ...
    def get_fig(self, title: str) -> mplf.Figure:
        self.fig.set_size_inches(4.5 * len(self.axs), 2.5 * len(self.axs[0]))
